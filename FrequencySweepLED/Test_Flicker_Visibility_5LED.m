% Test_Flicker_Visibility_5LED
%
% 2AFC Test that compares a static stimulus with a flickering stimulus.
% Flickering stimulus increases in frequency until the two are perceptually
% indistinguishable. 
%
...
%
% adapted from LEW 13/11/15
% written by MY 10/10/18


clear; close all; % Clear any data variables

% Add necessary folder to the path
addpath(genpath('/Users/miaomiaoyu/Documents/GitHub/fMRI/FrequencySweepLED'));

Connect_To_Arduino = 1; % Connect to arduino
...
dummyTrial(s); % Run a dummy  trial to prepare the stimulus (flashes once - also provides fixation point)

% Set some of the experimental parameters
dpy.NumSpec=3; % No. of assumed cones used to create
theExptID={'LMS'}; % Set the ID you want to test - was 'L'
...
freq=[25:0.5:35]; % Frequencies we want to test for each experiment ID: can be more than one...
freq=[1, 5];
...
dpy.NumTrialsPerLevel = 5; % The number of trials for each level - was 15, 5 for debugging
...
numShifts = length(dpy.shiftLevels); %number of frequency levels
a=1; %index
...
for thisCond = 1:TotalNumConds %for each condition
    ...
    cd('/Users/miaomiaoyu/Documents/GitHub/fMRI/FrequencySweepLED/DataFiles/IndividualConditions');
    ...
    cd('/Users/miaomiaoyu/Documents/GitHub/fMRI/FrequencySweepLED/IndividualConditions/Figures')
    %save figure
...
cd('/Users/miaomiaoyu/Documents/GitHub/fMRI/FrequencySweepLED/DataFiles/AllConditions')
%save data
